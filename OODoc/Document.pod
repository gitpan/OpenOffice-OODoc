...
OODoc::XPath (which do not need to be called directly by applications).
...
The OODoc::Document class itself only provides those methods (there are
very few) which cover the need to combine at least two or three constituent
modules. These additional methods are described in the present manual page.
...
=head3	Constructor : OpenOffice::OODoc::Document->new(<parameters>)
        ...
        Options are the same as for the OODoc::XPath constructor (and for its
	Text, Styles and Image derivatives). If no "member" parameter is
	given, the member selected by default is "content.xml".
