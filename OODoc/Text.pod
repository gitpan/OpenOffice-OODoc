...
=head3	getTable(number_or_name [, length, width])
	...
	use getTableByName() to avoid this behaviour;
...
=head3	getTableByName(name [, length, width])
	...
	allows the retrieval of a table whose name is made of digits and
	which, as a consequence, can't be properly selected with getTable().

	The behaviour and the optional arguments are the same as getTable().
