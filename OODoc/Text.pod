...
=head3  updateText(element [, options])
        ...
        the beginning; alternatively, 'text' may be a reference to a user-
        provided function whose return value will be used as the text to be
        inserted; this function will be called with the document object as its
        first argument (so it can use document-based methods) and optionally
        with a text string (that depends on other options, see below) as its
        second argument.
