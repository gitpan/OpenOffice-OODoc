...
such as spreadsheets or presentations as well as text documents.
...
=head3	Constructor : OpenOffice::OODoc::Text->new(<parameters>)
        ...
        creation method is like this:
            ...
            my $doc = ooText(file => 'my_file.sxw');
...
=head3	appendParagraph(<options>)
	...
	single space in the target document. See setText() and extendText() in
...
=head3	appendSection(name [, options])
	...
	The other possible option is "style". It allows the application to
	explicitly select a style for the new section.
...
=head3	getChapter(header_no [, options])
	...
	This method returns the list of the elements depending on a given
	header element. The argument and the options are the same as with
	getHeader().
...
=head3	getHeader(n [, options])
	...
	level[24]. E.g. if you have a level 1 header then two level 2 headers
...
=head3	insertSection(element, name [, options])
	...
	an existing element (paragraph, header, table). The referenced element
        can be indicated as in insertParagraph. The possible option is
	presently "style"; its value, if provided, is the style of the
	new section.
...
=head3	setSpan(element, [context,] expression, style)
	...
	"OpenOffice.org is my favourite word processor"
        ...
        presentation characteristics. "Style1" and "Style2" must be defined
	as text styles (not paragraph styles); these styles can be present
	in the source document or created (before or later) by the program
	(see createStyle() in OpenOffice::OODoc::Styles).
	
	It's possible, in a single setSpan() call, to give the same style
	to more than one text area in a text container. To do so, you just
	need to provide several text strings, separated by pipes ("|"), as
	the search expression. Example:
	
		$doc->setSpan($para, "OpenOffice|Writer", "MyStyle");
...
=head3	Other element methods
        ...
        getName method of XML::Twig::Elt [28]. Remember that all the
	features of XML::Twig::Elt are available for any text container.
