...
=head1	NOTES
	...
	named using aliases (see the Encode::Alias documentation).
        ...
        selectElementByAttribute and select the level attribute (see
        OODoc::XPath). The name of the level attribute is "text:level" in
	OOo documents and "text:outline-level" in OpenDocuments.
