...
There is a one-file documentation in OpenOffice.org (SXW) or PDF format, but
it's not maintained in sync with the distribution, so it's largely outdated.
However, it's available at the project page (http:://www.genicorp.fr/devel/oodoc) 
...
abstraction of the document, using a constructor like:

	OpenOffice::OODoc::object->new(options)

where "object" is generally "Document", for full access to the content, but
...
thanks to some predefined shortcut like "ooObject()". For example you can
write "ooDocument()" instead of "OpenOffice::OODoc::Document->new()".
...
can be used with spreadsheets (OpenOffice.org Calc documents) as well as
with tables included in text documents (however, there are a few more
constraints and performance issues with spreadsheets).
...
(Times 16 bold font, yellow background and navy blue foreground).
...
Starting from version 1.2, the default encoding can be selected by the
user during the installation, and changed later by editing a configuration
file.
...
Knowing that the tables are zero-based, so "A1" corresponds to (0,0). This
last notation is probably more user-friendly for OOo Calc documents.
...
In the last example, we must access both the text content and the
