...
OODoc::Intro - Introduction to Open Office Document Connector

=head1	DESCRIPTION

This introductory man page is intended to allow anybody to evaluate
some basic features of the OpenOffice::OODoc modules.

A full reference manual of the API is available as an OpenOffice.org
document (SXW format) at http://www.genicorp.fr/devel/oodoc
...
executed without any desktop software connection.
...
The OODoc module set is organized in 3 logical layers (a diagram is included
...
So we began to develop a third, more user-friendly layer.
...
=head2	Example 1: Dealing with metadata
...
related to a text content or an attribute value, like
	...
	$doc->selectParagraphsByStyle('Text body').
...
Among the examples provided in the package, you should have a look at
'oo2pod'. This simple 'extractor' arranges and extracts the content of an
OOo document and uses it to generate a POD document. (I use a more
complicated version of this script to generate the OpenOffice::OODoc
man pages, because the original document is written with the OpenOffice.org
text processor).
...
(the programmer must know what he does, for example in case of money or
...
=head2	Example 3 : Dealing with text AND metadata
...
=head2	Example 4 - Manipulating graphics
...
=head2	Example 5 - Managing styles
...
The examples and the test programs provided with the distribution.

=head1	CONCLUSION

There are probably some bugs to fix, a lot of new functionality to add,
and maybe some basic design to reconsider. But it reasonably works, and has
been practically used in small real-world projects.
