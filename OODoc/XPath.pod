...
=head3	Constructor : OpenOffice::OODoc::XPath->new(<parameters>); [4] 
        ...
        simultaneously could do it like this:
            ...
            my $archive	= OpenOffice::OODoc::File->new
            	("invoice.sxc");
            my $content	= OpenOffice::OODoc::XPath->new
            	(archive => $archive, member => 'content');
            my $styles	= OpenOffice::OODoc::XPath->new
            	(archive => $archive, member => 'styles');
        ...
        Optional parameters can also be passed to the constructor (see
        Properties below).
...
=head3	insertElement(element, name/xml [, options])
        ...
        The "attribute" option is itself a hash containing one or more
        attributes in the form [name => value] as in appendElement.
...
=head2	Properties
        ...
        The following are global variables which should be prefixed
        explicitly with $OpenOffice::OODoc::XPath:::
        ...
        encoding and whose default value is 'utf8'.

            LOCAL_CHARSET

        indicates the user's character set, by default 'ISO-8859-1'.

        The content of these three variables should not normally be
        modified, except the last one for purposes of internationalisation
        (see accepted values in the Perl Encode module's documentation).

        Instance hash variables are primarily those which were listed as
        parameters of the new constructor:
