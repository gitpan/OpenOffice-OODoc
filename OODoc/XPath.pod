...
element, i.e. the 13th paragraph (XPath occurrences are numbered
starting from 0).
...
=head3	appendElement(element, name/xml, [options]);
        ...
        document body. (Remember that in the case of an OpenOffice.org text
	...
	indicated in the styles list as it appears to the end user).
        ...
        encoded OpenOffice.org element. For example, it could be a
...
=head3	createSpaces(length)
	...
	text container. Same as blankSpaces(). See setText() and extendText().
...
=head3	getFlatText(element)
	...
	line breaks and repeated spaces. The returned text is a just a
	decoded flat string.
...
=head3	setText(element, text)
	...
	See blankSpaces() and extendText() for a workaround if you
...
=head3	splitElement(element, offset)
	...
	stops, "styled spans" or any kind of structure.
