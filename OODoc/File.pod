...
=head3	Constructor : OpenOffice::OODoc::File->new(<filename>)
        ...
        Example:
            ...
            my $archive	=
            	OpenOffice::OODoc::File->new("my_document.sxw");
...
=head3	ooTemplatePath([path])
	...
	The template directory must content "mimetype", "content.xml",
	"styles.xml", "meta.xml", "settings.xml" and "META-INF/manifest.xml".
	It's possible to change this requirement (for special uses and at the
	user's risk) by loading another member list in the appropriate table,
	i.e. @OpenOffice::OODoc::File::ARCHIVE_MEMBERS.
...
=head2	Properties
	...
	it can be set with the ooTemplatePath() function.

	The @OpenOffice::OODoc::File::ARCHIVE_MEMBERS contains the list of the
	template files needed to create a new file.
