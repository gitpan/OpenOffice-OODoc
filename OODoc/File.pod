...
=head3	Constructor : OpenOffice::OODoc::File->new(<filename>)
        ...
        during a save, by default "/tmp/" (if this parameter is set to null,
        the save workspace is the current user directory).

        Note: no content checking is carried out. The archive can be opened
        whether or not it is an OpenOffice.org document.

        The returned object, if successful, is a valid File instance, whose
        methods listed below become available.
