...
=head3	createImageElement(name [, options])
        ...
        beyond the end of the document or not. All that matters is that the
        particular page exists when it is opened by OpenOffice.org. If this
        parameter is absent, the image is anchored to a paragraph.
        ...
        physical page. If not given, the default is the top left corner of
        the associated paragraph or header. Coordinates go from left to
        right and top to bottom, however everything really depends on the
        image style. Coordinates should be given here in the form of a
        character string "x,y", and the default unit is centimetres. You can
        choose millimetres instead by attaching the usual abbreviation, such
        as "12.5cm, 35mm" which is the same as "125mm, 3.5cm" or "12.5,3.5",
        etc..
