...
operations on OpenOffice.org or OASIS OpenDocument files. It works without
...
from the details of the XML dialect and file format. It can process different
document classes (texts, spreadsheets, presentations, and drawings). It can
retrieve or update styles and images, global document properties (i.e.
metadata such as title, author, subject, etc), as well as text content.

OpenOffice::OODoc is object-oriented. A typical application generally
instantiates one or more document "connectors", each one bringing a lot of
access methods allowing easy read, update, delete, copy, import, and export
operations on a large set of content and presentation elements.

OpenOffice::OODoc is primarily designed in order to retrieve and/or update data
in existing documents, but it can generate new documents from scratch (or, more
...
Copyright 2006 by Genicorp, S.A. (http://www.genicorp.com)
